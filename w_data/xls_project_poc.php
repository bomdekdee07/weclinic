...
and v.visit_status <> 0 and v.proj_id=u.proj_id and u.proj_id = '$proj_id'
and u.uid_status IN(1,2) and v.visit_status=vs.status_id and v.visit_status <> 'C'
...
and v.visit_id <> 'SCRN' and v.visit_status NOT IN(0,10,'C')
...
/*
     ...
     p_project_uid_visit as v ,
     p_project_uid_list as u, uic_gen as u2
     WHERE t.uid=u.uid AND u.uid=u2.uid AND u.proj_id=?
     AND u.uid_status IN(1,2)
     AND v.uid=t.uid AND t.collect_date = v.visit_date AND v.visit_status <> 'C'
     ORDER BY collect_date";
*/
/*
     $query = "select distinct u.pid, u2.uic2 as uic, t.*
     from $prefix_db".$domain['id']." as t
     LEFT JOIN p_project_uid_list as u ON t.uid=u.uid and u.proj_id='POC' and (u.uid_status=1 OR u.uid_status=2)
     LEFT JOIN uic_gen u2 ON t.uid=u2.uid
     LEFT JOIN p_project_uid_visit as v ON t.uid=v.uid and t.collect_date=v.visit_date AND v.visit_status <> 'C'
     ORDER BY `u`.`pid` ASC ";
*/
     $query = "select distinct u.pid, u2.uic2 as uic, t.*
from p_project_uid_list as u, uic_gen as u2, $prefix_db".$domain['id']." as t
LEFT JOIN p_project_uid_visit as v ON t.uid=v.uid and t.collect_date=v.visit_date AND v.visit_status <> 'C'
WHERE t.uid=u.uid and u.proj_id=? and (u.uid_status=1 OR u.uid_status=2)
and t.uid=u2.uid
ORDER BY `u`.`pid` ASC ";
