...
<div id='div_hos_pid_list' class='div-hos-main mt-2'>
  <div class="row mt-1">
     <div class="col-sm-4">
       <h5><i class="fa fa-calendar-alt fa-lg" ></i> SAME DAY Hospital <b>Retrospective</b></h5>
     </div>
     <div class="col-sm-6 px-1">
       <small>
คนที่มีผลตรวจเอชไอวีเป็นบวก ที่ รพ. ของท่าน <b>ในช่วง 1 สิงหาคม 2560 ถึง 31 กรกฎาคม 2561</b>
      </small>
   ...
   <div class="row mt-0">
     <div class="col-sm-2">
       <label for="btn_hos_pid_export" class="text-light">.</label>
      <button class="btn btn-primary form-control" type="button" id="btn_hos_pid_export"><i class="fa fa-file-export" ></i> Data Export</button>
     ...
     <div class="col-sm-2">
       <label for="btn_hos_pid_clear_search" class="text-light">.</label>
       <button class="btn btn-warning form-control" type="button" id="btn_hos_pid_clear_search"><i class="fa fa-eraser" ></i> Clear Search</button>
     </div>
   </div>

  <div class="row mt-2">
     <div class="col-sm-3">
       <label for="search_hos_opt">เลือกโรงพยาบาล: </label>
       <select id="search_hos_opt" class="form-control" >

         <option value="" selected disabled>-เลือก-</option>
         <option value="%">All - ทั้งหมด</option>
         <option value="30">30 โรงพยาบาลเชียงใหม่ ฮอสพิทอล</option>
         <option value="31">31 โรงพยาบาลนครพิงค์</option>
         <option value="32">32 โรงพยาบาลสันป่าตอง</option>
         <option value="33">33 โรงพยาบาลสารภี</option>
         <option value="34">34 โรงพยาบาลหางดง</option>
         <option value="20">20 โรงพยาบาลเชียงรายประชานุเคราะห์</option>
         <option value="50">50 โรงพยาบาลสรรพสิทธิประสงค์</option>
         <option value="40">40 โรงพยาบาลหาดใหญ่</option>
         <option value="10">10 โรงพยาบาลสมเด็จพระบรมราชเทวี ณ ศรีราชา</option>
         <option value="11">11 โรงพยาบาลวัดญาณสังวรวราราม</option>
...
<div id='div_hos_pid_detail' class="div-hos-main mt-1">
  <div class="row bg-primary text-white">
     ...
     <div class="col-sm-6">
        <button type="button" class="btn btn-primary btn-hos-mnu" data-id="1"><i class="fa fa-user fa-lg" ></i> Info</button>
        <button type="button" class="btn btn-primary btn-hos-mnu" data-id="2"><i class="fa fa-notes-medical fa-lg" ></i> Baseline</button>
  ...
  <div>
    ...
    <div id='div_hos1' class='div-hos-menu'>
       <?  include_once("retro_pid_personal_data.php"); ?>
    </div>
    <div id='div_hos2' class='div-hos-menu'>
       <?  include_once("retro_pid_hos_baseline.php"); ?>
    </div>
...
//  alert("xx "+s_clinic_id);
  ...
  $("#btn_hos_pid_export").click(function(){
     exportData_hos_pid();
  }); // btn_hos_pid_export
...
//alert("final "+flag_close);
  ...
  $("#btn_hos_pid_clear_search").click(function(){
     clearSearch();
  }); // btn_hos_pid_clear_search
});

function clearSearch(){
  $("#search_hos_opt").val("%");
  $("#search_hos_opt2").val("");
  $("#txt_search_hos_pid").val("");

}

function searchData_hos_pid(){

      if($('#search_hos_opt').val() == null){
        $('#search_hos_opt').notify("กรุณาเลือกโรงพยาบาล", "info");
        return;
      }
      var aData = {
                u_mode:"select_hos_pid_list_center",
                txt_search:$('#txt_search_hos_pid').val(),
                search_hos_opt:$('#search_hos_opt').val(),
                search_hos_opt2:$('#search_hos_opt2').val()

      };
      save_data_ajax(aData,"w_proj_SDHOS/db_hos_pid_retro.php",searchData_hos_pidComplete);
        ...
        for (i = 0; i < datalist.length; i++) {
          ...
          if(dataObj.im == "0"){ // current data
            if(dataObj.art_pass == "1"){
              ...
              txt_btn_log += ' <td><button class="btn btn-block btn-'+rtn_color+'" type="button" onclick="goHosMnu(\''+dataObj.pid+'\', \'3\')""><i class="fa fa-stethoscope"></i> Retention  '+txt_rtn+txt_rtn_date+'</button></td>';
              txt_btn_log += ' <td><button class="btn btn-block btn-'+ae_color+'" type="button" onclick="goHosMnu(\''+dataObj.pid+'\', \'4\')""><i class="fa fa-procedures"></i> AE '+txt_ae+'</button></td>';
            }
            else{
              txt_btn_log += ' <td>-</td><td>-</td>';
              ...
              txt_btn_base='<button class="btn btn-block btn-'+base_color+'" type="button" onclick="goHosMnu(\''+dataObj.pid+'\', \'2\')""><i class="fa fa-notes-medical"></i> Base Line</button>';
          }
          else{// old record
            txt_btn_base='ข้อมูลเดิม';
            var rtn_color = "primary"; var ae_color = "primary";
            var txt_rtn = ""; var txt_ae = "";
            if(dataObj.rtn_pend > 0) {
              txt_rtn = " <small>("+dataObj.rtn_pend+")</small>";
            ...
            txt_row += '<tr class="r_hos_pid '+flag_done+'" id="r_'+dataObj.pid+'" data-ul="'+dataObj.ul+'" data-log="'+dataObj.art_pass+'" data-create="'+dataObj.create_date+'" data-name="'+dataObj.name+'">';
            txt_row += ' <td>'+dataObj.pid+'</td>';
            txt_row += ' <td>'+dataObj.ul+'</td>';


            txt_row += ' <td>'+txt_btn_base+'</td>';
...
function goHosMnu(pid, choice){
  ...
  cur_hos_pid_data_log = $('#r_'+pid).data("log");
  ...
  $('#title_hos_pid_detail').html("PID: "+cur_hos_pid+" / UL: "+cur_hos_ul);
    ...
    function checkExistLogData(logForm, visitDate){
      //alert("checkExistLogData 1 ");
        ...
        save_data_ajax(aData,"w_proj_SDHOS/db_hos_pid_retro.php",checkExistLogDataComplete);
...
function exportData_hos_pid(){
  if($('#search_hos_opt').val() == null){
    ...
    return;
      ...
      save_data_ajax(aData,"w_proj_SDHOS/xls_sdhos_data_export_retro.php",exportData_hos_pidComplete);
...
function exportData_hos_pid_all(){
      ...
      save_data_ajax(aData,"w_proj_SDHOS/xls_sdhos_data_export_all_retro.php",exportData_hos_pidComplete);
...
/*
      function getMaxSeqNo(logForm){ // get max seq_no to insert new log record (eg. AE)
        //alert("checkExistLogData 1 ");
          ...
          save_data_ajax(aData,"w_proj_SDHOS/db_hos_pid_retro.php",getMaxSeqNoComplete);
