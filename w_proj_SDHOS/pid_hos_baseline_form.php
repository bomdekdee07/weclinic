...
$after_save_function = "
if(typeof rtnDataAjax.flag_success !== 'undefined' ){
  aftersaveBaseLine();
}
else{
  alert('ขัอมูลบันทึกไม่สมบูรณ์ กรุณากดบันทึกใหม่อีกรอบ');
  $.notify('ขัอมูลบันทึกไม่สมบูรณ์ กรุณากดบันทึกใหม่อีกรอบ', 'error');
  return;
}
...
$("#acceptSDART-N").click(function(){ // acceptSDART-N
  ...
  if(is_pass_baseline()){ //pass baseline criteria
    ...
    var hide_q = '<? echo "$q_physical_exam,$q_lab,$q_referral,diag_date,start_ART,ART_regimen,Not_ART_title,Not_ART_oi_txt,Not_ART_lab_txt,startART_date"; ?>';
    ...
    var i;
     ...
     var hide_t = '<? echo "$t_physical_exam,$t_lab,$t_referral,artinitial_title"; ?>';
