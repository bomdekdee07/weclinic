...
$search_hos_opt = isset($_POST["search_hos_opt"])?$_POST["search_hos_opt"]:"%";

$query_add = " s.clinic_id like '$search_hos_opt' ";
$query_add .= " AND (s.pid LIKE '$txt_search'  OR s.ul LIKE '$txt_search' OR s.hn LIKE '$txt_search' ) ";



$file_name = "export_sdhos_retro_$sc_id";
...
$inQuery_PID = " pid IN (SELECT s.pid from sdhos_pid_retro as s
where $query_add
)";
...
$inQuery = "select COLUMN_NAME from information_schema.COLUMNS where TABLE_NAME='sdhos_sh_retro' AND TABLE_SCHEMA='$db_name'";
//echo "<br><b>".$inQuery."</b>";
$stmt = $mysqli->prepare($inQuery);
$stmt->execute();
...
$inQuery_PID_baseline = " pid IN (SELECT s.pid from sdhos_pid_retro as s
 where $query_add AND is_import=0
) ";

$query = "select *
from sdhos_sh_retro as s
...
$web_path = "w_proj_SDHOS/data/$file_name";
