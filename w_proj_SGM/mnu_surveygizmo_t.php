...
#tbl_surveygizmo_list th {
    background-color:#000;
    color:#FFF;
}
#tbl_surveygizmo_list {
    width: 100%;
    margin: 0 auto;
    position: relative;
}
#tbl_surveygizmo_list tr.scroll {
    display: table;
    width: 100%;
    position: absolute;
}
...
function AncorControlMatches() {
    topPos = $('#tbl_surveygizmo_list').offset().top,
     pos = $(document).scrollTop(),
     total = pos - topPos;
    if (pos > topPos) {
        $("#tbl_surveygizmo_list thead tr").css("top", total).addClass('scroll');
    } else {
        $("#tbl_surveygizmo_list thead tr").removeClass('scroll');
    }
}
...
function submitReviseSG_FormDone(c_uic, c_visit_date){
  ...
  //  alert("revise uic:"+revise_uic);
    save_data_ajax(aData,"w_proj_SGM/db_surveygizmo.php",submitReviseSG_FormDoneComplete);
  }
...
function saveRemark(c_uic, c_visit_date){
  ...
  if($('#txtfrm'+rowID).val().trim() == ""){
    ...
    return;
      ...
      save_data_ajax(aData,"w_proj_SGM/db_surveygizmo.php",saveRemarkComplete);
