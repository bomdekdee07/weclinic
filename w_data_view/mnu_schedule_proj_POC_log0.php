...
$txt_thead = '  <thead>
            <tr>
              <th class="bg-primary text-white fixleft-col"><i class="fa fa-user" ></i> PID</th>
              <th>UIC</th>
              <th>UID</th>
...
.tbl-fixleft thead, .tbl-fixleft tbody {
    display: block;
}
  ...
  <div class="card-body">
    <div class="card-title">
      <div class="row">
         ...
         <div class="col-sm-4">
            <b><span id="txt_poc_group"></span></b>
         </div>
         <div class="col-sm-2">
      ...
      <div class="row mt-4 bg-info">
         <div class="col-sm-3">
           <button class="btn btn-info form-control btn_schedule_poc" type="button" data-id="001">
             <i class="fa fa-users" ></i> กลุ่ม 1 เริ่ม PrEP
           </button>
         </div>
         <div class="col-sm-3">
           <button class="btn btn-info form-control btn_schedule_poc" type="button" data-id="002">
             <i class="fa fa-users" ></i> กลุ่ม 2 กิน PrEP
           </button>
         </div>
         <div class="col-sm-3">
           <button class="btn btn-info form-control btn_schedule_poc" type="button" data-id="003">
             <i class="fa fa-users" ></i> กลุ่ม 3 ไม่กิน PrEP
           </button>
         </div>
         <div class="col-sm-3">
           <button class="btn btn-info form-control btn_schedule_poc" type="button" data-id="004">
             <i class="fa fa-users" ></i> กลุ่ม 4 ผลเลือดบวก
           </button>
         </div>
    ...
    <div id="div_poc_group" class="poc_schedule mt-2">
      <table id="tbl_poc_group"  class="tbl-fixleft ">
          <? echo $txt_thead; ?>
          <tbody>
            <tr class="r_poc_sd"><td colspan=28 align="center"><b><h4>- กรุณาเลือกกลุ่ม -</h4></b></td></tr>
          </tbody>
      </table>
      <div class="my-3">
        <!--
...
//  $(".poc_schedule").hide();
  ...
  $(".btn_schedule_poc").click(function(){
    getDataPOC_Schedule($(this).data("id"));

  //  showDivPOC_Schedule($(this).data("id"));
...
function getDataPOC_Schedule(groupID){
  ...
  if(flagSave){
    ...
    if(datalist.length > 0){
      ...
      for (i = 0; i < datalist.length; i++) {
        ...
        txt_row += '<td class="fixleft-col bg-primary text-white"><b><button class="btn btn-primary btn-poc-visit" data-uid="'+dataObj.uid+'" data-uic="'+dataObj.uic+'" data-pid="'+dataObj.pid+'">'+dataObj.pid+'</button></b></td>';
        txt_row += '<td><b>'+dataObj.uic+'</b></td>';
        txt_row += '<td><b>'+dataObj.uid+'</b></td>';
        txt_row += '<td>'+dataObj.enroll+'</td>';
        ...
        // schedule_data
        txt_row += '<td class="poc_blue">'+dataObj.M0s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M0st)+'">'+dataObj.M0v_date+'</td>';
        txt_row += '<td class="poc_lightblue">'+dataObj.M0s_note+'</td>';

        txt_row += '<td class="poc_blue">'+dataObj.M1s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M1st)+'">'+((dataObj.M1st!='10')?dataObj.M1v_date:"ไม่มาตามนัด")+'</td>';
        txt_row += '<td class="poc_lightblue">'+dataObj.M1s_note+'</td>';

        txt_row += '<td class="poc_blue">'+dataObj.M3s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M3st)+'">'+((dataObj.M3st!='10')?dataObj.M3v_date:"ไม่มาตามนัด")+'</td>';
        txt_row += '<td class="poc_lightblue">'+dataObj.M3s_note+'</td>';

        txt_row += '<td class="poc_blue">'+dataObj.M6s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M6st)+'">'+((dataObj.M6st!='10')?dataObj.M6v_date:"ไม่มาตามนัด")+'</td>';
        txt_row += '<td class="poc_lightblue">'+dataObj.M6s_note+'</td>';

        txt_row += '<td class="poc_blue">'+dataObj.M9s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M9st)+'">'+((dataObj.M9st!='10')?dataObj.M9v_date:"ไม่มาตามนัด")+'</td>';
        txt_row += '<td class="poc_lightblue">'+dataObj.M9s_note+'</td>';

        txt_row += '<td class="poc_blue">'+dataObj.M12s_date+'</td>';
        txt_row += '<td class="'+getVisitClassColor(dataObj.M12st)+'">'+((dataObj.M12st!='10')?dataObj.M12v_date:"ไม่มาตามนัด")+'</td>';
      ...
      $('.r_poc_sd').remove(); // remove old poc schedule row
         ...
         txt_row += "<tr><td colspan=15 align='center'>-ยังไม่มีข้อมูล-</td></tr>";
    }
