      ...
      var txt_row = "";
        for (i = 0; i < datalist.length; i++) {
            ...
            addRowData_SpecimenCollectList(
             dataObj.qid, dataObj.uid, dataObj.wait_lab_result, dataObj.c_date, dataObj.c_time, dataObj.cf_time, dataObj.doctor_name,dataObj.qrd,dataObj.staff_order_room
            );
        }//for
...
function addRowData_SpecimenCollectList(qid, uid, wait_lab_result, collect_date, collect_time, confirm_order_time, confirm_by, qrd, order_room){
  ...
  if(isRequireFix){
    ...
    if(!isRequireFix) txt_row += ' onclick="openSPC(\''+qid+uid+'\');">';
