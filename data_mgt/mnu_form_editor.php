     ...
     <div class="col-sm-6" id="div_form_editor_search">
          <table width='100%'>
            ...
            <td align="right">
              ...
              <button class="bg-warning btn-form-component" type="button" data-id="html">HTML</button>
              <button class="bg-warning btn-form-component" type="button" data-id="q_label">Q Label</button>
              <button class="bg-warning btn-form-component" type="button" data-id="line">Line</button>
              <button class="bg-info btn-form-component" type="button" data-id="logform">Log Form</button>
              <button class="bg-info btn-form-component" type="button" data-id="iframe">iFrame</button>
              <button class="bg-info btn-form-component" type="button" data-id="includepage">includepage</button>
             </div>
            </td>
            </tr>
...
//  console.log("key "+event.which);
  ...
  function openFormView(){
     let formID = $("#form_editor").attr("data-formid");
      ...
      // var link = "mnu_form_view.php?form_id="+formID+"&lang=th&uid=TEST&collect_date=0000-00-00&collect_time=00:00:00";
      // var link = "mnu_form_view.php?form_id="+formID+"&lang=th&uid=TEST&collect_date=0000-00-00&collect_time=00:00:00";
       var link = "pribta21/ext_index2.php?file=p_form_view&form_id="+formID+"&lang=th&uid=TEST&collect_date=0000-00-00&collect_time=00:00:00";
  ...
  //  $('.r_data').remove();
    ...
    function searchData_DataList(){
      ...
      save_data_ajax(aData,"data_mgt/db_form_editor.php",searchData_DataList_Complete);
    }
  ...
  $("#txt_search_data_item").val("");
   ...
   function openFormEditor(formID){ // openFormEditor
     ...
     $('.txt_form_name').html("["+$('#r'+formID).find('td:nth-child(1)').text()+"] "+$('#r'+formID).find('td:nth-child(2)').text());
       ...
       save_data_ajax(aData,"data_mgt/db_form_editor.php",openFormEditor_Complete);
   }
         ...
         data_list.forEach(function(itm) {
          // console.log("data: "+itm.data_id);
           //addRow_DataForm(data_id, data_name,  data_type, data_seq, data_sub_list){
            if(itm.data_type == 'q_label' || itm.data_type == 'html' || itm.data_type == 'logform' || itm.data_type == 'iframe' || itm.data_type == 'includepage' || itm.data_type == 'line')
            addRow_ComponentForm(itm.data_id, itm.data_type ,itm.data_seq, itm.data_value, itm.data_value_en);
            else{
...
//console.log('addRow_DataForm '+data_id+'/'+is_require);
...
function updateReq(dataID){
  ...
  if($("#c"+dataID).is(':checked')){
    ...
    function addRow_ComponentForm(data_id, data_type, data_seq, data_value, data_value_en){
      data_value = getShowText(data_value); data_value_en = getShowText(data_value_en);


      let classRow = '';
      var btn_update = '<button class="bg-info text-white" type="button" onclick="updateCompVal(\''+data_id+'\');"> Update Seq & Text</button>';
      var comp = ""; var btn_attr="";

      if(data_type == "q_label"){

        classRow = 'bg-warning';
        comp = '<div><input type="text" id="v'+data_id+'" data-odata="'+data_value+'" data-col="data_value" class="form-control form-control-sm c'+data_id+'" value="'+data_value+'" placeholder="ใส่ q_label ภาษาไทยตรงนี้" /></div>';
        comp+= '<div><input type="text" data-odata="'+data_value_en+'" data-col="data_value_en" class="form-control form-control-sm pdata-en c'+data_id+'" value="'+data_value_en+'" placeholder="put q_label English version here" /></div>';

      }
      else if(data_type == "html"){
        comp = '<div><textarea id="v'+data_id+'" data-odata="'+data_value+'" data-col="data_value" class="form-control form-control-sm c'+data_id+'"  rows="4" placeholder="ใส่ html แบบภาษาไทยตรงนี้">'+data_value+'</textarea></div>';
        comp+= '<div><textarea data-odata="'+data_value_en+'" data-col="data_value_en" class="form-control form-control-sm pdata-en c'+data_id+'"  rows="4" placeholder="put html English version here">'+data_value_en+'</textarea></div>';

      }
      else if(data_type == "logform"){
        classRow = 'bg-msoft2';
        comp = '<div><input type="text" id="v'+data_id+'" data-odata="'+data_value+'" data-col="data_value" class="form-control form-control-sm c'+data_id+'" value="'+data_value+'" placeholder="Insert log form title." /></div>';
        btn_attr += '<button class="bg-info text-white" type="button" onclick="openDataAttr(\''+data_id+'\');"> Attr. </button>';

      }
      else if(data_type == "iframe" || data_type == "includepage"){
        classRow = 'bg-msoft2';
        comp = '<div><input type="text" id="v'+data_id+'" data-odata="'+data_value+'" data-col="data_value" class="form-control form-control-sm c'+data_id+'" value="'+data_value+'" placeholder="Insert title" /></div>';
        btn_attr += '<button class="bg-info text-white" type="button" onclick="openDataAttr(\''+data_id+'\');"> Attr. </button>';
      ...
      var txt_row = '<div class="row r_form_data fdata-comp '+classRow+'" id="r'+data_id+'" data-id="'+data_id+'"  data-type="'+data_type+'"  >' ;
    ...
    function getShowText2(sObjValue){
    			var	result = "";
...
function addFormComponent(dataType){ // addFormComponent
    ...
    save_data_ajax(aData,"data_mgt/db_form_editor.php",addFormComponent_Complete);
}
...
function addFormComponent_Complete(flagSave, rtnDataAjax, aData){
    if(flagSave){
      ...
      //  console.log("dataobj: "+dataObj.data_id+"/"+dataObj.data_seq);
        addRow_ComponentForm(dataObj.data_id, dataObj.data_type ,dataObj.data_seq, "");

        scrollToDataItem($('#div_form_editor_head').children().last().attr("id"));
        if(dataObj.data_type == "html"){
...
function addFormDataItem(str_data_item){ // addFormDataItem
    ...
    save_data_ajax(aData,"data_mgt/db_form_editor.php",addFormDataItem_Complete);
...
function updateCompVal(dataID){ // update data component (data_seq, data_value)
   ...
   if(flag_update){
         ...
         save_data_ajax(aData,"data_mgt/db_form_editor.php",updateCompValComplete);
   }
   else{
...
function deleteFormData(dataID){ // update data component (data_seq, data_value)
   ...
   if(result){
         ...
         save_data_ajax(aData,"data_mgt/db_form_editor.php",deleteFormDataComplete);
   }
...
include_once("mnu_form_editor_data_prop2.php");
?>
