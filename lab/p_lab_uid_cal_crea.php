...
$sColtime= isset($_GET["coltime"])?$_GET["coltime"]:"";
  ...
  $query = "SELECT count(distinct uid) as count_check
  from p_data_result 
  where uid = ?
  and collect_date = ?
  ...
  $stmt->bind_param($param_val, ...$array_val);

  if($stmt->execute()){
    $stmt->bind_result($count_check);
    while($stmt->fetch()){
      $data_check_project = $count_check;
    }
    // echo "TEST:".$data_check_project;
  }
  $stmt->close();

  $txt_uid_info = "";
  if($data_check_project != "1"){
    $query = "SELECT P.uic, P.sex, P.date_of_birth,  P_HGT.data_result as height, P_WGT.data_result as weight, PLR.lab_result as serum
    FROM patient_info P
    LEFT JOIN p_data_result P_HGT ON (P_HGT.uid=P.uid AND P_HGT.data_id='heigh' AND P_HGT.collect_date = ?)
    LEFT JOIN p_data_result P_WGT ON (P_WGT.uid=P.uid AND P_WGT.data_id='cn_weight' AND P_WGT.collect_date=P_HGT.collect_date AND P_WGT.collect_time=P_HGT.collect_time)
    LEFT JOIN p_lab_result PLR ON (PLR.uid=P.uid AND PLR.lab_id='CREA' AND PLR.collect_date=P_HGT.collect_date AND PLR.collect_time=P_HGT.collect_time)
    WHERE P.uid = ?
    ORDER BY PLR.collect_date desc , P_HGT.collect_time desc  LIMIT 1 ";
  }
  else{
    $query = "SELECT P.uic, P.sex, P.date_of_birth,  P_HGT.data_result as height, P_WGT.data_result as weight, PLR.lab_result as serum
    FROM patient_info P
    LEFT JOIN p_data_result P_HGT ON (P_HGT.uid=P.uid AND P_HGT.data_id='heigh' AND P_HGT.collect_date = ?)
    LEFT JOIN p_data_result P_WGT ON (P_WGT.uid=P.uid AND P_WGT.data_id='cn_weight' AND P_WGT.collect_date=P_HGT.collect_date AND P_WGT.collect_time=P_HGT.collect_time)
    LEFT JOIN p_lab_result PLR ON (PLR.uid=P.uid AND PLR.lab_id='CREA' AND PLR.collect_date=P_HGT.collect_date)
    WHERE P.uid = ?
    ORDER BY PLR.collect_date desc , P_HGT.collect_time desc  LIMIT 1 ";
  }
	...
	if($stmt->execute()){
      ...
      if($affect_row > 0){
          ...
          $txt_uid_info = "
             <div class='fl-wrap-row ptxt-s12 px-1 div_crea'
               ...
               <div class='fl-fill ptxt-b'>
                  ...
                  <div class='fl-wrap-row ph-50'>
										<div class='fl-fix fl-mid pw100 pbg-blue ptxt-white pbtn btn-update-uiddata'>
													Update Values
										</div>
                  ...
                  <div class='fl-wrap-row ph-50 bg-ssoft2 ptxt-b py-1'>
                     ...
                     <div class='fl-fix pw100'>
                           Weight (kgs):<br> <input type='number' size='7' class= 'crea-calc wgt savedata' data-id='cn_weight'  data-odata='$wgt'value='$wgt' \>
                     </div>
                     <div class='fl-fix pw100'>
                           Hight (cm):<br> <input type='number' size='7' class= 'crea-calc hgt savedata' data-id='heigh' data-odata='$hgt' value='$hgt' \>
                     </div>
                     <div class='fl-fix pw100'>
...
$(document).ready(function(){
 ...
 $(".btn-update-uiddata").click(function(){ // update uid data
	 let sLstdata = [];
	 $('.savedata').each(function(ix,objx){
       if($(objx).val() != ''){
				 console.log("enter 0 "+$(objx).val()+"/"+ $(objx).attr('data-odata'));
				 if($(objx).val() != $(objx).attr('data-odata')){
					 let dataObj = {};
					 dataObj[$(objx).attr('data-id')] = $(objx).val();
					 sLstdata.push(dataObj);
				 }

       }
   });

	 if(sLstdata.length > 0){
		 let sUID =$(".div_crea").attr("data-uid");
		 let sColdate =$(".div_crea").attr("data-coldate");
		 let sColtime =$(".div_crea").attr("data-coltime");

			var aData = {
				 u_mode:"update_data_from_crea_calc",
				 uid:sUID,
				 coldate:sColdate,
				 coltime:sColtime,
				 lst_data:sLstdata
