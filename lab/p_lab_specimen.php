...
$(document).ready(function(){
  ...
  sWait = false;
  $("#tbl_specimen_list").on("click",".btnspeccancel",function(){
      if(sWait==true){
        return;
      }
      sWait=true;
      ...
      sSpecId = $(this).attr("data-specid");

      if(confirm("Do you want to cancel this specimen?")){
        var aData = {
            u_mode:"store_specimen",
            bar:sBar,
            specid:sSpecId,
            uid:sUid,
            coldate:sColDate,
        ...
        save_data_ajax(aData,"lab/db_lab_speciment_save.php",labsavecomplete);
      }

  });
  function labsavecomplete(flagSave, rtnDataAjax, aData){
    if(flagSave){
        if(aData.u_mode=="store_specimen"){
          $(".btnspeccancel[data-uid='"+aData.uid+"'][data-coldate='"+aData.coldate+"'][data-coltime='"+aData.coltime+"'][data-barcode='"+aData.bar+"'][data-specid='"+aData.specid+"']").parent().html("Specimen Store");
        }
    }
    sWait = false;
  }
...
*/
        ...
        datalist_specimen_barcode.forEach(function (itm) {
          ...
          txt_row += " <td>";
          txt_row +=  itm.status +" "+ ((itm.specimen_status!="S3" && itm.specimen_status!="S4")?"<button class='btnspeccancel' data-uid='"+cur_spc_uid+"' data-coldate='"+cur_spc_date+"' data-coltime='"+cur_spc_time+"' data-barcode='"+itm.barcode+"' data-specid='"+itm.specimen_id+"'>X</button>":"");
          txt_row += " </td>";
          txt_row += "</tr>";
