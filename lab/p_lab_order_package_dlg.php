...
$aLabPkg['papsmear']['name'] = "Pap smear (Sure Prep)"; 
/*
...
*/
$aLabPkg['papsmear']['lab_id'] = array('SURE_PREP','SURE_PREPC','SURE_PREPN');
...
$aLabGroupID['WBC'] = array('BA%', 'EO%', 'Hb', 'HCT', 'LY%', 'MCH', 'MCHC', 'MCV', 'MO%', 'NE%', 'PLT', 'RBC', 'RDW-CV', 'WBC');
// OLD CODE close 09/02/2023 // array('BA#', 'BA%', 'EO#', 'EO%', 'Hb', 'HCT', 'LY#', 'LY%', 'MCH', 'MCHC', 'MCV', 'MO#', 'MO%', 'MPV', 'NE#', 'NE%', 'PLT', 'RBC', 'RDW-CV', 'RDW-SD', 'WBC' );
...
function showLabPackage($pkg_id, $pkg_color='bg-mdark1', $pkg_color_lab='bg-msoft1' ){
  ...
  */
     ...
     foreach($aLabPkg[$pkg_id]['lab_id'] as $lab_id){
      ...
      // $txt_row .= "  < class='fl-fix px-1 p-row pw250'>";
      // $txt_row .= "   <label class='pbtn'> <input type='checkbox' class='chk-lab v-checkbox' data-id='".$lab_id."' data-pkg='$pkg_id'> ".$lab_name."</label>"; | $lbt $sale_opt
       ...
       $txt_row .= "    <div class='fl-fix w-235'>";
       ...
       // BOM add BTN Enable group
       $txt_row .= "    <div class='fl-fix w-45 fl-mid'>
                          <button id='btn_pck_group_lab' class='btn font-s-1 btn_pck_group_lab' data-pkgid='$pkg_id' style='padding: 0px 3px 0px 3px;'>Setting</button>
                        </div>";
       $txt_row .= "    <div class='fl-fill fl-mid-right px-1 pw15 ptxt-b ptxt-s8 pbtn pbtn-cancel btn-lab-remove' data-id='$lab_real_id' title='Remove from order' $hideRemoveBtn>X</div>";
       $txt_row .= "    </div>";
       $txt_row .= "  ";
     } // foreach
    ...
    //   $spc_txt .= "  <div class='fl-float px-1 mr-1 pbtn pbtn-blue'>$spc</div>";
     ...
     $txt_row .= "  <div class='fl-fix ph30 fl-mid'>".$pkg_id."</div>";
...
.lab-row{
  ...
  <script src="../weclinic/asset/js/pribta.js?t<? echo("=".time()); ?>"></script>
...
function showPKG(pkg_id, is_show){
  ...
  $('.div-lab-request .btn-select-lab-order-dlg').on("click",function(){
    ...
    $(".div-lab-request .chk-lab:checked").each(function(ix,objx){
      package_id += $(objx).attr("data-pkg")+"%";
      package_id_one = $(objx).attr("data-pkg");
      if($(objx).prop("disabled")){}
      else{
        let arr_data_lab_id = $(objx).attr('data-labid').split(':');
        arr_data_lab_id.forEach(function(lab_id){
          // console.log("addLab: "+lab_id+" package:"+package_id);
          lst_data_obj.push([package_id_one, lab_id]);
        });
  ...
  // BT Setting group
  $(".div-lab-request .btn_pck_group_lab").off("click");
  $(".div-lab-request .btn_pck_group_lab").on("click", function(){
    var pkg_id = $(this).attr("data-pkgid");
    var sSid = $('.div-lab-request').attr("data-sid");
    let sUID=$('.div-lab-request').attr("data-uid");
    let sColdate=$('.div-lab-request').attr("data-coldate");
    let sColtime=$('.div-lab-request').attr("data-coltime");
    var sUrl_appoint = "lab/p_lab_order_package_setting_main.php?package_id="+pkg_id+"&sid="+sSid+"&uid="+sUID+"&coldate="+sColdate+"&coltime="+sColtime;
    showDialog(sUrl_appoint, "Package Setting", "20%", "46%", "", function(sResult){}, false, function(sResult){});
  });
    ...
    if(confirm('Are you sure to remove '+labid_show+' from this order?')){
      ...
      callAjax("lab/db_lab_test_order.php",aData,function(rtnObj,aData){
          ...
          if(rtnObj.res == 1){
            ...
            // After delete update p_lab_order=> [package_lab_id] again
            var sPackage_id = "";
            $(".div-lab-request .chk-lab:checked").each(function(ix,objx){
              sPackage_id += $(objx).attr("data-pkg")+"%";
            });
            ...
            let sColtime=$('.div-lab-request').attr("data-coltime");
            var aData = {
              u_mode: "update_p_lab_order_package",
              uid: sUID,
              coldate: sColdate,
              coltime: sColtime,
              package_id: sPackage_id
            };

            $.ajax({
              url: "lab/p_lab_order_package_ajax.php",
              cache: false,
              type: "POST",
              data: aData,
              success: function(sResult){
                var rtn = $.parseJSON(sResult);
                if(rtn.success){
                  console.log(rtn.success);
                  var btnremove = $('.div-lab-request');
                  btnremove.notify("Lab Removed.", "success");
                }
              }
            });
