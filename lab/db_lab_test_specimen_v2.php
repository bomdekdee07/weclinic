...
//echo "query1: $uid, $collect_date, $collect_time / $query";
  ...
  $bind_param = "sss";
  $array_val = array($obj_data["uid"],  $obj_data["collect_date"],  $obj_data["collect_time"]);
  $statsu_order = "";
  ...
  $stmt->close();
  // echo "test0: ".$statsu_order;

  if($statsu_order == "A2" && $statsu_order != "" ){
    $bind_param = "sss";
    $array_val = array($obj_data["uid"],  $obj_data["collect_date"],  $obj_data["collect_time"]);
    $cont_old_labid = array();

    $query = "SELECT DISTINCT lab_group_id
    from p_lab_test 
    where lab_id in (
      select lab_id 
    ...
    if($stmt->execute()){
      $result = $stmt->get_result();
      while($row = $result->fetch_assoc()){
        $cont_old_labid[$row["lab_group_id"]]["group_name"] = $row["lab_group_id"];
      }
    }
    $stmt->close();
    // echo "test1: ".count($cont_old_labid)."<br>";
    // print_r($cont_old_labid);

    if(count($cont_old_labid) != 0){
      $bind_param = "sss";
      $array_val = array($obj_data["uid"],  $obj_data["collect_date"],  $obj_data["collect_time"]);
      $old_status = "";
      ...
      and collect_date = ?
      and collect_time = ?
      order by upd_date DESC, seq_lab DESC;";
      $stmt = $mysqli->prepare($query);
      $stmt->bind_param($bind_param, ...$array_val);
      ...
      $str_sql_group = "";
      foreach($cont_old_labid AS $group_id=>$val){
        $str_sql_group .= "'".$group_id."',";
      }
      // echo "test3: ".substr($str_sql_group, 0, -1);

      $bind_param = "sss";
      $array_val = array($obj_data["uid"],  $obj_data["collect_date"],  $obj_data["collect_time"]);
      $cont_barcode_labid = array();

      $query = "SELECT pro_spec.lab_group_id
      from p_lab_order_specimen order_spc
      join p_lab_order_specimen_process pro_spec on(pro_spec.barcode = order_spc.barcode)
      where order_spc.uid = ?
      and order_spc.collect_date = ? 
      and order_spc.collect_time = ?
      and pro_spec.lab_group_id in (".substr($str_sql_group, 0, -1).");";
      $stmt = $mysqli->prepare($query);
      $stmt->bind_param($bind_param, ...$array_val);
      ...
      if($stmt->execute()){
        $result = $stmt->get_result();
        while($row = $result->fetch_assoc()){
          $cont_barcode_labid[$row["lab_group_id"]] = $row["lab_group_id"];
        }
      }
      $stmt->close();
      // echo "test2: ".count($cont_barcode_labid)."<br>";
      // print_r($cont_barcode_labid);

      if(count($cont_old_labid) == count($cont_barcode_labid)){
        $sUid = $obj_data["uid"];
        $sColdate = $obj_data["collect_date"];
        $sColtime = $obj_data["collect_time"];
        $query = "UPDATE p_lab_order set lab_order_status = '$old_status' where uid = '$sUid' and collect_date = '$sColdate' and collect_time = '$sColtime'";
        $stmt = $mysqli->prepare($query);
        if($stmt->execute()){
          addToLog("update status p_lab_order [$sUid|$sColdate|$sColtime]|status:$old_status", $s_id);
        }
        $stmt->close();
        // echo "query:".$query;
      }
        ...
        $stmt = $mysqli->prepare($query);
        if($stmt->execute()){
          addToLog("update status p_lab_order [$sUid|$sColdate|$sColtime]|status:A2", $s_id);
        }
        $stmt->close();
      }
    }
  ...
  //error_log('Pending amt: '.$arr_data[0]["pending_amt"] );
    if($arr_data[0]["pending_amt"] == 0){
      ...
      $query = "SELECT QL.queue as queue, o.wait_lab_result
        ...
        // Close function by BOM from Clinic 08/06/2023
...
else if($u_mode == "edit_specimen_collect_time"){ // update_specimen_time
  $uid = isset($_POST["uid"])?$_POST["uid"]:"";
  $collect_date = isset($_POST["coldate"])?$_POST["coldate"]:"";
  $collect_time = isset($_POST["coltime"])?$_POST["coltime"]:"";
  $specimenid = isset($_POST["specimenid"])?$_POST["specimenid"]:"";
  $barcode_s = isset($_POST["barcodeS"])?$_POST["barcodeS"]:"";
  $specimencoltime = isset($_POST["specimencoltime"])?$_POST["specimencoltime"]:"";
  $affect_row = 0;
  $query = "UPDATE p_lab_order_specimen
  SET time_specimen_collect=?
  WHERE uid=? AND collect_date=? AND collect_time=? AND specimen_id=? AND barcode = ?
  ";

  //  error_log("$specimen_receive,$specimen_collect,  $uid,$collect_date,$collect_time / $query ");
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('ssssss', $specimencoltime, $uid,$collect_date,$collect_time, $specimenid, $barcode_s);
    if($stmt->execute()){
      $affect_row = $stmt->affected_rows;
      if($affect_row > 0) {
        $res = 1;
        addToLog("[p_lab_order_specimen] edit collect time [$uid|$collect_date|$collect_time|$specimenid] $specimencoltime", $s_id);
      }
    }
    else{
      error_log($stmt->error);
