...
ResetTimeOutTimer();
 ...
 cur_setting_choice = "sale_option"; // current setting choice eg. lab_specimen, lab_testing_menu
 cur_setting_col_id = "sale_opt_id"; // current setting table eg. lab_specimen, lab_testing_menu
 cur_setting_title = "Sale Option (ชนิดราคาขาย)";
</script>
...
<div id='div_setting_detail' class="div-setting-menu my-0" style="display:none;">
  ...
  <div id='div_setting_detail_form' >
    <div class=" my-1" style="background-color:#eee;">
      <center>
      Record ID: <input type="text" id="<? echo $setting_col_id; ?>" size="20" disabled>
    </center>
    </div>
    <div class="row my-1">
      <div class="col-sm-12">
        <label for="sale_opt_name">Sale Option Name:</label>
        <input type="text" id="sale_opt_name" data-title="Name"  class="form-control form-control-sm save-data v-no-blank " maxlength="200">
      </div>
...
function addSettingData(){
  ...
  $("#"+cur_setting_col_id).val("ADD NEW");
...
function saveSettingDataComplete(flagSave, rtnDataAjax, aData){
  ...
  if(flagSave){
   if(u_mode_setting == "add_setting"){
     ...
     $('#'+cur_setting_col_id).val(cur_setting_id);

   }
   else{
     // remove updated row  to replace with new data
     $('#r'+cur_setting_choice+$('#'+cur_setting_col_id).val()).remove();
   }
   var arr_obj = [$('#sale_opt_name').val()];
   var txt_row = addRowData_Setting(
      $('#'+cur_setting_col_id).val(),
      arr_obj
   );
...
function searchDatasetting_Complete(flagSave, rtnDataAjax, aData){
  ...
  function openSettingData_Complete(flagSave, rtnDataAjax, aData){
    ...
    if(flagSave){
        ...
        $('#'+cur_setting_col_id).val(dataObj.id);
        $('#sale_opt_name').val(dataObj.name);
  ...
  function deleteSettingData(){
    var result = confirm("ท่านต้องการลบข้อมูล "+$('#'+cur_setting_col_id).val()+" นี้ใช่หรือไม่ ?");
    if (result) {
      var aData = {
