<?
    ...
    $coldate = isset($_GET["coldate"])?$_GET["coldate"]:"";
    $coltime = isset($_GET["coltime"])?$_GET["coltime"]:"";
    $doc_datetime = isset($_GET["doc_datetime"])?$_GET["doc_datetime"]:"";
    // echo $pk."/".$uid."/".$coldate;

    $check_bt = isset($_POST["check_bt"])?$_POST["check_bt"]:"";
    $date_print_fix = isset($_POST["fix_date_print_now"])?$_POST["fix_date_print_now"]:""; //"28/08/2023 11:11 by พัชรินทร์ เอี่ยมยัง";
    ...
    $query = "SELECT o.lab_order_id,
        ...
        o.staff_lab_save AS staff_lab_save,
        o.staff_lab_save_license AS staff_lab_save_license,
        o.staff_confirm AS staff_confirm,
        o.staff_confirm_license AS staff_confirm_license,
        o.time_lab_report_confirm,
        o.hide_patient_name,
        o.hide_project
    FROM log_p_lab_order_his o
    where pk_log_p_lab_order = ?
    and uid = ?
    and collect_date = ?
    and collect_time = ?
    and doc_datetime = ?";
    ...
    $stmt->bind_param("sssss",$pk, $uid, $coldate, $coltime, $doc_datetime);
    ...
    $stmt->close();
    // print_r($arr_data_lab_order);
    
    $lab_order_id = $arr_data_lab_order["lab_order_id"];
    $hidename = $arr_data_lab_order["hide_patient_name"];
    $hideproject = $arr_data_lab_order["hide_project"];

    // specimen lab result
    $arr_specimen = array();
    $query = "SELECT LOS.specimen_name,
        LOS.specimen_transform,
        LOS.specimen_id,
        LOS.time_specimen_collect,
        LOS.lab_group_id,
        LOS.laboratory_id 
    FROM
        log_p_lab_specimen_his LOS
    WHERE LOS.uid= ? AND LOS.collect_date = ?  AND LOS.collect_time = ? AND LOS.pk_log_p_lab_order = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ssss",$arr_data_lab_order['uid'],
    ...
    $query = "SELECT
        ...
        PLR.lab_group_id,
        PLR.laboratory_id,
        PLR.specimen_transform,
        PLR.staff_save AS staff_save,
        PLR.staff_save_license AS staff_save_license,
        PLR.staff_confirm AS staff_confirm,
        PLR.staff_confirm_license AS staff_confirm_license,
        PLR.staff_print_by AS staff_print_by,
        PLR.time_lab_confirm,
        PLR.m_lab_std_txt AS m_lab_std_txt,
        PLR.m_min AS m_min,
        PLR.m_max AS m_max,
        PLR.f_lab_std_txt AS f_lab_std_txt,
        PLR.f_min AS f_min,
        PLR.f_max AS f_max 
    FROM
        log_p_lab_result_his PLR
    WHERE
        PLR.uid = ?
        AND PLR.collect_date = ?
        AND PLR.collect_time = ?
        AND PLR.pk_log_p_lab_order = ?
    ORDER BY
        PLR.lab_group_id,
        PLR.lab_id2;";
    ...
    $bind_param = "ss";
    $array_val = array($arr_data_lab_order["proj_id"], $arr_data_lab_order["proj_visit"]);
    $timepoint_name = "";

    $query = "SELECT timepoint_desc
    ...
    if($stmt->execute()){
        $result = $stmt->get_result();
        while($row = $result->fetch_assoc()){
        $timepoint_name = $row["timepoint_desc"];
        }
    }
    $stmt->close();
    ...
    else if($arr_data_lab_order["proj_id"] == "PURPOSE2"){
        $convert_lab_proj_name_SDART = $arr_data_lab_order["proj_id"];

        if($timepoint_name != ""){
            $visit_SDART = $timepoint_name;
        }
        else{
            $visit_SDART = $arr_data_lab_order["proj_visit"];
        }
    ...
    $date_normal = (new DateTime())->format('d/m/Y H:i')." by ".$staff_print_by;
    ...
    <table>
        <tr>
            ...
            <td style='padding-top:15px;'>
                ...
                <div style='font-size:4px;color:white;'></div>
    ...
    <div>
        <table width='100%' style='border-top: 1px solid #000;border-bottom: 1px solid #000; font-size:10px;'>
            ...
            <tr>
                <td width='50%' height='20px'>Approved by: $staff_confirm $staff_confirm_license $time_confirm </td><td>Printed Date and Time: ".($check_bt=="1"? $date_print_fix: $date_normal)."</td>
            </tr>
        </table>
    </div>
    ...
    **** FINAL REPORT, Please File ****
    ...
    if($arr_data_lab_order["en_fname"] != ""){
        $patient_name_en .= "
            <tr>
                <td width='20%'>Patient Name (Eng):</td><td width='30%'>".$arr_data_lab_order["en_fname"]." ".$arr_data_lab_order["en_sname"]."</td>
            </tr>";
    }
    ...
    <table width='100%' class='general-info'>
        <tr>
            <td width='20%'>Patient Name (TH):</td><td width='30%'>".$patient_name."</td>
