...
$hideproject = isset($_GET["hideproject"])?$_GET["hideproject"]:"";
  ...
  //echo "<br>query2: ".$arr_data_lab_order['uid']."/".$arr_data_lab_order['collect_date']."/".$arr_data_lab_order['collect_time']." / $query";
      ...
      if($stmt->execute()){
        ...
        $tmp_age = floor($tmp_diff / (365*60*60*24));
         ...
         $tmp_remarks = 0;
         if($row["lab_result_note"] != "" && $lab_test_name == "Creatinine Clearance"){
            $tmp_remarks = explode("\n", $row["lab_result_note"]);
           }
         ...
         if($row["lab_name"] == 'Creatinine, serum/plasma'){
          ...
          if($row['ext_lab'] == 0){
            ...
            if($lab_test_name == "Creatinine Clearance"){
            $txt_row_result.="<td width='25%' class='lab-row'>".$lab_test_name."<br><span class='lab-method'>".$row["lab_method_name"].$lab_test_specimen."</span><br/>(Cockcroft-Gault equation)</td>";
            }else{
            ...
            if(count($tmp_remarks) > 0 && $lab_test_name == "Creatinine Clearance"){
