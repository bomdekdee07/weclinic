...
$form_id = "covid_screen_g2";
$form_name = "COVID Screening Group 2 ";
...
function checkScreenPass_Covid(){
   var is_pass = 'Y';
   if($("#covid_near-N").prop("checked") == true){
     is_pass = 'N';
   }
...
function updateScreenIDComplete(flagSave, rtnDataAjax, aData){
  if(flagSave){
        ...
        if($("#is_pass").val() == "Y"){
          ...
          var link = "f_form_proj.php?proj_id=COVID&form_id=covid_visit_g<? echo $group_id; ?>&link="+rtnDataAjax.link;
          //alert("link "+link);
