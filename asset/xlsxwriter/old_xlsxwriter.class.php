...
class XLSXWriter
{
	...
	public function writeSheetHeader($sheet_name, array $header_types, $suppress_row = false, $use_row_as_head = true)
	{
	...
	public function writeSheetHeaderX($sheet_name, array $header_types, $suppress_row = false)
	{
		if (empty($sheet_name) || empty($header_types) || !empty($this->sheets[$sheet_name]))
			return;
		self::initializeSheet($sheet_name);
		...
		foreach($header_types as $v)
		{
			$sheet->columns[] = $this->addCellFormat("string");
		}
        if (!$suppress_row)
        {
			//$header_row = array_keys($header_types);
