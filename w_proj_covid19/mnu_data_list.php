...
<div id='div_covid_list' class='div-covid-menu mt-0 '>
  <div class="row mt-4">
     <div class="col-sm-3">
           ...
           <div id="div_paper_input" style="display:none;">
             <input type="text" id="txt_paper_date" size="20">
             <button type="button"  id="btn_covid_new_paper" ><i class="fa fa-file fa-lg" ></i> กรอกกระดาษ (ย้อนหลัง)</button>
           </div>
...
function addRowData(u_mode, pid,screen_id, collect_date, staff_id, is_online){
    ...
    if(pid != ''){// pass screen
      if(u_mode == "select_pid"){
        ...
        if($is_delete == "1"){
          //echo "btn_delete = '<button class=\"btn btn-primary btn-sm \" type="button" data-pid="'+pid+'"  onclick="opencovid_FormMain(\''+screen_id+'\',\''+collect_date+'\',\''+is_online+'\',\'1\')""><i class="fa fa-user" ></i> คัดกรอง ['+screen_id+']</button>';';";
          $is_delete_display = '';
        }
        ?>
        btn_delete = '<button class="btn btn-danger btn-sm " type="button" onclick="delete_covid_data(\''+pid+'\', \''+screen_id+'\')" <? echo $is_delete_display;?> ><i class="fa fa-times" ></i> ลบ</button>';
...
function delete_covid_data(pid_param, screenID){
  ...
  if (result) {
    ...
    save_data_ajax(aData,"w_proj_covid19/db_covid.php",delete_covid_data_Complete);

  }

}

function delete_covid_data_Complete(flagSave, rtnDataAjax, aData){
  //alert("flag save is : "+flagSave);
  if(flagSave){
