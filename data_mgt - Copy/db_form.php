...
//echo "umode : $u_mode";
 if($u_mode == "select_form_dropdown"){ // select_form_dropdown (for dropdown)
  ...
  if($count_data_row == 0){
    $sqlCmd = "INSERT INTO p_form_list_data (form_id, data_seq, data_id, data_type, data_value, is_require)
      SELECT ?, data_seq, data_id, data_type, data_value, is_require FROM p_form_list_data WHERE form_id=?
      ON DUPLICATE KEY UPDATE data_seq=VALUES(data_seq), data_type=VALUES(data_type),data_value=VALUES(data_value), is_require=VALUES(is_require);
    ";
