...
if($u_mode == "select_form_data_item"){ // select_form_data_item
    ...
    $query = " SELECT fd.data_id,  CONCAT(d.data_name_th, '/', d.data_name_en) as data_name,
      fd.data_value,fd.data_seq, fd.data_type, fd.is_require
...
//echo " data: ".$data_item['data_id'];
        ...
        $data_question = $data_item['data_question_th'];
...
*/
  ...
  else if($u_mode == "update_data_attr"){ // update_data_attr
      $data_id = isset($_POST["data_id"])?$_POST["data_id"]:"";
      $form_id = isset($_POST["form_id"])?$_POST["form_id"]:"";
      $data_obj = isset($_POST["data_obj"])?$_POST["data_obj"]:[];

      $sPrepare = ""; $aDataObj = array(); $affect_row=0;
      $query = "REPLACE INTO p_form_list_data_attribute (form_id, data_id, attr_id, attr_val) VALUES ";
      foreach($data_obj as $attr_id=>$attr_val){
        $query .= "(?,?,?,?),";
        $sPrepare .="ssss";
        array_push($aDataObj,$form_id);
        array_push($aDataObj,$data_id);
        array_push($aDataObj,$attr_id);
        array_push($aDataObj,$attr_val);
      }// foreach
      if($query != ""){
        $query = substr($query,0,strlen($query)-1) ;
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param($sPrepare,...$aDataObj);
