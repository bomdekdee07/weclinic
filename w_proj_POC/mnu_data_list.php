...
function addNewSUT(){ // add patient to screen
     if($('#txt_new_sut').val().trim().length > 7){
       ...
       save_data_ajax(aData,"w_proj_SUT/db_sut.php",addNewSUTComplete);
     }
     else{
...
function addRowData(uid,uic,pid,screen_date, visit_date, content_type,  s_consent,is_confirm_consent, send_date, receive_date, test_date, destroy_date, test_no  ){
    var btn_screen = "";
    var btn_visit="";
    var btn_followup="";
...
//alert("ocntent type "+content_type);
    if(content_type == 1){
      ...
      if(visit_date == ''){ // not done form
        ...
        content_type = 'กระดาษ <button  onclick="openSUT_FormLink(\'1\',\''+uid+'\',\''+uic+'\',\''+screen_date+'\')"><i class="fa fa-user" ></i> <b>ส่งฟอร์ม Online </b> </button>';
      ...
      if(visit_date == ''){ // not done form
        //send form link to do
        content_type = 'ออนไลน์ <button  onclick="openSUT_FormLink(\'2\',\''+uid+'\',\''+uic+'\',\''+screen_date+'\')"><i class="fa fa-user" ></i> <b>ส่งฟอร์ม Online '+screen_date+'</b> </button>';
        ...
        btn_follow_disable = "disable";
          ...
          if(is_confirm_consent == '0'){
             btn_group = '<div id="cfm'+uid+screen_date+'" class="btn-group btn-group-sm mb-1" role="group" aria-label="confirm"><button class="btn btn-success btn-sm px-1" type="button" onclick="confirm_consent(\'1\', \''+uid+'\', \''+screen_date+'\',\''+visit_date+'\' );"><i class="fa fa-check" ></i> ยืนยัน</button>';
             btn_group += '<button class="btn btn-danger btn-sm px-1" type="button" onclick="confirm_consent(\'0\', \''+uid+'\', \''+screen_date+'\',\''+visit_date+'\' );"><i class="fa fa-times" ></i> ยกเลิก</button></div>';
          }
        content_type = btn_group+'<div><button class="btn btn-primary btn-sm " type="button" data-uid="'+uid+'"  onclick="openSUT_FormMain(\'0\',\''+uid+'\',\''+uic+'\',\''+pid+'\',\''+screen_date+'\',\''+visit_date+'\',\''+test_no+'\')"><i class="fa fa-user" ></i> ออนไลน์ </button></div>';
      }
    }
    ...
    btn_visit = '<button class="btn btn-warning btn-sm " '+btn_visit_disable+' type="button" data-uid="'+uid+'"  onclick="openSUT_FormMain(\'2\',\''+uid+'\',\''+uic+'\',\''+pid+'\',\''+screen_date+'\',\''+visit_date+'\',\''+test_no+'\')""><i class="fa fa-user" ></i> แบบฟอร์ม </button>';
    ...
    var content_type_id = 'c'+uid+screen_date;
...
var row_content = "";
    ...
    row_content += ' <td><div id="'+content_type_id+'">'+content_type+'</div></td>';
  ...
  var txt_row = '<tr class="r_sut " id="r_'+uid+screen_date+'" data-uid="'+uid+'" data-uic="'+uic+'" data-pid="'+pid+'"';
...
function confirm_consent(choice, uid_param, screenDate, visitDate){
  var flag_ok = 0;
  if(choice == '0'){
  ...
  if(flag_ok == 1){
    ...
    save_data_ajax(aData,"w_proj_SUT/db_sut.php",confirm_consent_Complete);
  }
...
function searchData_sut(){
      ...
      save_data_ajax(aData,"w_proj_SUT/db_sut.php",searchDataSUT_Complete);
...
function searchDataSUT_Complete(flagSave, rtnDataAjax, aData){
  ...
  if(flagSave){
    ...
    if(rtnDataAjax.datalist.length > 0){
      ...
      $('.r_sut').remove(); // row pid list
      txt_row += '<tr class="r_sut"><td colspan="7" align="center">ไม่พบข้อมูล</td></tr">';
      $('#tbl_sut_pid_list > tbody:last-child').append(txt_row);
    }
...
function openSUT_FormLink(choice, uid, uic, visit_date){ // choice 1:paper, 2:online
  ...
  var open_link = "w_proj_SUT/link_form.php?";
...
function afterScreen_FormOnline(){
      ...
      save_data_ajax(aData,"w_proj_SUT/db_sut.php",afterScreen_FormOnlineComplete);
}
...
function afterScreen_FormPaper(){
  //alert("afterscreen_paper");
      ...
      save_data_ajax(aData,"w_proj_SUT/db_sut.php",afterScreen_FormPaperComplete);
}
...
function exportData_SUT_PRE(){
      ...
      save_data_ajax(aData,"w_proj_SUT/xls_sut_pre_data_export.php",exportData_SUT_PRE_Complete);
