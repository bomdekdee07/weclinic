...
// specimen lab result
$arr_specimen = array();
$query = "SELECT SP.specimen_name, SP.specimen_transform, LOS.specimen_id, LOSP.lab_group_id, LOSP.laboratory_id
...
if($stmt->execute()){
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $arr_specimen[$row['lab_group_id'].$row['laboratory_id']] = $specimen;
      ...
      if($stmt->execute()){
        ...
        while($row = $result->fetch_assoc()) {
          ...
          if($row['specimen_transform'] != '') $lab_test_specimen = "| <i>".$row['specimen_transform']."</i>";
              ...
              $lab_test_specimen = " | <i>".$arr_specimen[$row['lab_group_id'].$row['laboratory_id']]."</i>";
          }
...
if($hidename != '1') $patient_name = $arr_data_lab_order["fname"]." ".$arr_data_lab_order["sname"];
...
<td width='20%'>Collected time:</td><td width='30%'>".$arr_data_lab_order["lab_specimen_collect"]."</td>
</tr>
...
<tr >
<td class='lab-row-header'><b>Test Name</b><br><span class='lab-method'>Lab Method | <i>Specimen</i></span></td>
