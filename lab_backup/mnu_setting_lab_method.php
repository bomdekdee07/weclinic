...
ResetTimeOutTimer();
 ...
 cur_setting_choice = "lab_method"; // current setting choice eg. lab_specimen, lab_testing_menu
 cur_setting_col_id = "lab_method_id"; // current setting table eg. lab_specimen, lab_testing_menu
 cur_setting_title = "LAB Method ";
</script>
...
<div id='div_setting_detail' class="div-setting-menu my-0" style="display:none;">
  ...
  <div id='div_setting_detail_form' >
    <div class=" my-1" style="background-color:#eee;">
      <center>
      Record ID : <input type="text" data-id="<? echo $setting_col_id; ?>" id="s<? echo $setting_col_id; ?>" size="20"  disabled>
    </center>
    </div>
    <div class="row my-1">
      <div class="col-sm-12">
        <label for="slab_method_name">Name:</label>
        <input type="text" id="slab_method_name" data-id="lab_method_name" data-title="Name"  class="form-control form-control-sm save-data v-no-blank " maxlength="150">
      </div>

     </div>
     <div class="row my-1">
       <div class="col-sm-12">
         <label for="slab_method_note">Note:</label>
         <textarea id="slab_method_note" rows="4" data-id="lab_method_note"  data-title="Note" class="form-control save-data" placeholder="กรอกหมายเหตุ"></textarea>
       </div>
...
if(is_modal_select == 1)$('#txt_search_setting').val(cur_component_act.val());
...
//  alert("<? echo $setting_col_id; ?> / cur  col "+cur_setting_col_id);
  $("#div_setting_detail .save-data").val("");
  $("#s"+cur_setting_col_id).val("ADD NEW");
  $('#slab_method_name').focus();
}
  ...
  if(validateInput(divSaveData)){
   ...
   $("#"+divSaveData +" .save-data").each(function(ix,objx){
     //alert("grab : "+$(objx).attr("id"));
      lstDataObj.push({name:$(objx).data("id"), value:$(objx).val()});
      //lstDataObj.push({name:$(objx).attr("id"), value:$(objx).val()});

   });


  // alert("size : "+lstDataObj.length+"/umodesetting "+u_mode_setting);
    var aData = {
...
function saveSettingDataComplete(flagSave, rtnDataAjax, aData){
  ...
  if(flagSave){
   ...
   var arr_obj = [$('#slab_method_name').val()];
...
function searchDatasetting_Complete(flagSave, rtnDataAjax, aData){
  ...
  function openSettingData_Complete(flagSave, rtnDataAjax, aData){
    ...
    if(flagSave){
        ...
        $('#slab_method_name').val(dataObj.name);
        $('#slab_method_note').val(dataObj.note);
