  ...
  <div class="card-body" >
    <div class="row my-1">
      ...
      <form method="post" action="" enctype="multipart/form-data" id="myform" class="lab-part" style="margin-left:30px;display:inline;">
          ...
          <input type="file" id="filePDF" name="file" accept=".pdf" title='Less than 8MB only' /><input type="button" class="button" value="Upload" style='display:none' id="btnUploadPdf"  <? echo("data-uid='".$uid."' data-coldate='".$collect_date."' data-coltime='".$collect_time."'") ?> />
      </form>
          ...
          <table id="tbl_lab_test_result" class="table table-bordered table-sm table-striped table-hover">
              <thead>
                <tr>
                  ...
                  <th>Lab Test     <button id="btn_sel_print_lab_result" class="my-1 bg-mdark2 text-white" type="button" style='margin-right:30px'>
                        <i class="fa fa-print fa-lg" ></i> Print Selected Lab
                      </button></th>
...
$(".div-table-lab-report #btn_sel_print_lab_result").on("click",function(){
...
/*
   console.log("submitttt "+$('#div_lab_result').attr('data-oid'));
  let lablist = [];
  $(".div-table-lab-report .chk-lab-report").each(function(ix,objx){
     if($(objx).prop('checked')) lablist.push($(objx).val());
  });

  var fd = new FormData();
  fd.append('oid', $('#div_lab_result').attr('data-oid'));
  fd.append('printid', $('#div_lab_result').attr('data-sid'));
  fd.append('lablist', lablist);
  callAjaxForm("p_form_file_a.php",fd,function(){
...
function printLabTestReport(){
  ...
  var link = "link_lab_report.php?lab_order_id="+cur_lab_order_id+"&sp_id="+specimen_id+"&uid="+uid+"&visit="+visit+hidename;
