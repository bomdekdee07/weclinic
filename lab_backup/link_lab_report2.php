...
*/
      ...
      /*
      $query = "SELECT PLOS.specimen_id, PLS.specimen_name,
      PLT.lab_id2, PLR.lab_id, PLT.lab_name, PLR.lab_serial_no,
      ...
      FROM p_lab_order_specimen PLOS
LEFT JOIN p_lab_specimen PLS
ON PLOS.specimen_id = PLS.specimen_id
LEFT JOIN p_lab_result PLR
ON PLR.uid = PLOS.uid
AND PLR.collect_date = PLOS.collect_date
AND PLR.collect_time = PLOS.collect_time
AND PLR.barcode=PLOS.barcode
...
ON PLP.lab_serial_no = PLR.lab_serial_no
...
WHERE PLTRH.start_date <= now() AND PLR.external_lab != 1 AND PLTRH.stop_date > now() AND PLP.lab_process_status='P1'
AND PLOS.uid = ? AND PLOS.collect_date = ? AND PLOS.collect_time = ? ". $query_add;
*/
...
WHERE PLTRH.start_date <= now() AND PLR.external_lab != 1 AND PLTRH.stop_date > now()
...
/*
$query_test = "<br>query2: ".$arr_data_lab_order['uid']."/".$arr_data_lab_order['collect_date']."/".$arr_data_lab_order['collect_time']." / $query";
