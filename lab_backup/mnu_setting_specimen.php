...
ResetTimeOutTimer();
 ...
 cur_setting_choice = "specimen"; // current setting choice eg. lab_specimen, lab_testing_menu
 cur_setting_col_id = "specimen_id"; // current setting table eg. lab_specimen, lab_testing_menu
 cur_setting_title = "Specimen (สิ่งส่งตรวจ)";
</script>
...
<div id='div_setting_detail' class="div-setting-menu my-0" style="display:none;">
  ...
  <div id='div_setting_detail_form' >
    <div class=" my-1" style="background-color:#eee;">
      <center>
      Record ID: <input type="text" id="s<? echo $setting_col_id; ?>" size="20" disabled>
    </center>
    </div>
    <div class="row my-1">
      <div class="col-sm-9">
        <label for="specimen_name">Name:</label>
        <input type="text" id="specimen_name" data-title="Name"  class="form-control form-control-sm save-data v-no-blank " maxlength="150">
      </div>
      <div class="col-sm-3">
        <label for="specimen_unit">Unit:</label>
        <input type="text" id="specimen_unit" data-title="Unit" class="form-control form-control-sm save-data" maxlength="20">
      </div>
     </div>
     <div class="row my-1">
       <div class="col-sm-12">
         <label for="specimen_note">Note:</label>
         <textarea id="specimen_note" rows="4"  data-title="Note" class="form-control save-data" placeholder="กรอกหมายเหตุ"></textarea>
       </div>
...
$(document).ready(function(){
  $('#modal_setting').on('shown.bs.modal', function() {
    $('#txt_search_setting').trigger('focus');
  });
...
function saveSettingDataComplete(flagSave, rtnDataAjax, aData){
  ...
  if(flagSave){
   ...
   var arr_obj = [$('#specimen_name').val(),$('#specimen_unit').val()];
...
function searchDatasetting_Complete(flagSave, rtnDataAjax, aData){
    ...
    if(rtnDataAjax.datalist.length > 0){
      ...
      var txt_row = "";
        for (i = 0; i < datalist.length; i++) {
            ...
            var arr_obj = [dataObj.name,dataObj.unit];
  ...
  function openSettingData_Complete(flagSave, rtnDataAjax, aData){
    ...
    if(flagSave){
        ...
        $('#specimen_name').val(dataObj.name);
        $('#specimen_unit').val(dataObj.unit);
        $('#specimen_note').val(dataObj.note);
