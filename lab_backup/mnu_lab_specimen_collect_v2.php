...
function searchData_SpecimenCollect(){
  ...
  save_data_ajax(aData,"lab/db_lab_test_specimen_v2.php",searchData_SpecimenCollect_Complete);
    ...
    if(rtnDataAjax.datalist.length > 0){
      ...
      var txt_row = "";
        for (i = 0; i < datalist.length; i++) {
            ...
            */
            addRowData_SpecimenCollectList(
             dataObj.queue, dataObj.uid, dataObj.wait_lab_result, dataObj.c_date, dataObj.c_time, dataObj.cf_time, dataObj.doctor_name,dataObj.room_no,dataObj.fname,dataObj.sname,dataObj.dob
            );
        }//for
...
function addRowData_SpecimenCollectList(queue, uid, wait_lab_result, collect_date, collect_time, confirm_order_time, confirm_by, order_room,fname,sname,dob){
  dob_th = changeToThaiDate(dob);
  isRequireFix = false;

  confirm_order_time = (confirm_order_time==null)?"":confirm_order_time;
  ...
  if(isRequireFix){
    ...
    sFixBtn = confirm_by+" (Room: "+order_room+")";
  }

    var txt_row = '<tr class="rspc" id="rspc'+queue+uid+'"  data-uid="'+uid+'" data-collect_date="'+collect_date+'" data-collect_time="'+collect_time+'" data-queue="'+queue+'" data-wait_lab_result="'+wait_lab_result+'">' ;
