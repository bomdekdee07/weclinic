...
if($u_mode == "select_hos_pid_list"){ // select_pid_list
  ...
  $query_add_clinic = "";
  if($clinic_id != "%")
  $query_add_clinic = " AND s.clinic_id = '$clinic_id' ";
  ...
  $arr_pending_retention = array(); // ยังกรอกฟอร์มไม่ครบ retention
  $query = "SELECT s.pid, count(r.pid) as num
  FROM sdhos_pid as s, sdhos_sh_retention as r
  WHERE s.pid = r.pid AND
  (r.pid, r.collect_date)  NOT IN (
    select d.pid, d.collect_date
    from sdhos_form_done as d , sdhos_pid as s
  ...
  //echo "$clinic_id, $schedule_date/ $query";
      ...
      $arr_pending_ae = array(); // ยังกรอกฟอร์มไม่ครบ ae
      $query = "SELECT s.pid, count(r.pid) as num
      FROM sdhos_pid as s, sdhos_sh_ae as r
      WHERE s.pid = r.pid AND
...
//echo "$clinic_id, $schedule_date/ $query";
       ...
       $query = "SELECT s.pid, count(r.pid) as num
       FROM sdhos_pid as s, sdhos_sh_baseline as r
       WHERE s.pid = r.pid AND
       (r.pid)  NOT IN (
         select d.pid
         from sdhos_form_done as d , sdhos_pid as s
         where d.form_id='sdhos_baseline' $query_add
       ) $query_add
       GROUP BY r.pid
       ORDER BY r.pid
              ";
 //echo "$clinic_id, $schedule_date/ $query";
  ...
  b.arv_previous, b.arv_previous_place, b.startART_date
  FROM sdhos_pid as s
  LEFT JOIN sdhos_sh_baseline as b ON (s.pid=b.pid)
  ...
  //echo "$clinic_id, $schedule_date/ $query";
         ...
         if($stmt->execute()){
           $stmt->bind_result($pid, $ul, $f_name, $s_name, $hn, $create_date, $is_import, $arv_previous, $arv_previous_place, $art_start_date );
           ...
           while ($stmt->fetch()) {
             ...
             if($is_import == "0"){ // current record
               if($arv_previous == "Y"){
                 if($arv_previous_place != "2") $art_pass = "0";
                 else {
                   if($art_start_date !== NULL && $art_start_date != '0000-00-00')
                   $art_pass = "1";
                 }
               }
               else if($arv_previous == "N"){
                 if($art_start_date !== NULL && $art_start_date != '0000-00-00')
                 $art_pass = "1";
               }
             }
             else{ // old record (import from screen log)
                 $art_pass = "1";
             }
...
//echo "$clinic_id, $schedule_date/ $query";
 ...
 //echo "$clinic_id, $schedule_date/ $query";
  ...
  $query = "SELECT s.pid, s.ul, s.f_name, s.s_name, s.create_date,s.is_import,
  b.arv_previous, b.arv_previous_place, b.startART_date
  FROM sdhos_pid as s
  LEFT JOIN sdhos_sh_baseline as b ON (s.pid=b.pid)
  WHERE  (s.pid LIKE ?  OR s.ul LIKE ? OR s.f_name LIKE ? OR s.s_name LIKE ? OR s.hn LIKE ? )
  ...
  //echo "$clinic_id, $schedule_date/ $query";
         ...
         $stmt->bind_param("sssss", $txt_search, $txt_search, $txt_search, $txt_search, $txt_search);
         if($stmt->execute()){
           $stmt->bind_result($pid, $ul, $f_name, $s_name, $create_date, $is_import, $arv_previous, $arv_previous_place, $art_start_date );
  ...
  if($count_duplicate == 0){
    ...
    $query.= " ,?,?,?,?,?,?,?,?,?,?, now(), ?";
    $query.= " FROM sdhos_pid WHERE SUBSTRING(pid,1,$where_substr_pos_end) = '".$id_prefix."-';";
  //echo "$query" ;
...
else if($u_mode == "get_pid_age"){ // get age to baseline
...
$mysqli->close();
 ...
 function checkDuplicateCitizenID($strPID, $strCitizenID){
       ...
       FROM sdhos_pid WHERE citizen_id = ?";
