        ...
        for (i = 0; i < datalist.length; i++) {
            ...
            addRowData_LabOrder(
             ...
             dataObj.status_id, dataObj.status_name, dataObj.wait_lab_result,
              dataObj.request_by, dataObj.room_no, dataObj.clinic_type ,dataObj.time_confirm_order,dataObj.time_specimen_collect,dataObj.time_lab_order_pmt,dataObj.time_lab_report_confirm,dataObj.proj_id,dataObj.proj_pid,dataObj.proj_visit
...
function addRowData_LabOrder(lab_order_id,  uid, collect_date, collect_time,
  status_id, status_name,wait_lab_result, request_lab_by, requested_room_no, clinic_type,time_confirm_order,time_specimen_collect,time_lab_order_pmt,time_lab_report_confirm,projid,pid,vid
  ,queue, room_no, prev_room ){
    ...
    if(queue != null){
      ...
      if(room_no == '24'){
          send_queue_txt += '<br><button class="btn btn-sm btn-warning" onclick="sendQueueDlg(\''+lab_order_id+'\');">ส่งคิว | Send Queue</button>';
      }
    }
...
/*
    ...
    else if(wait_lab_result=='0'){
    //   wait_lab_result = '<span class="text-danger"><b>No</b></span>';
    wait_lab_result = '<span class="text-danger"><b>No</b></span><br>';
    //  wait_lab_result +='<button class="btn btn-info" type="button" onclick="sendQueue(\''+lab_order_id+'\');"> Send Queue</button>';
    wait_lab_result +='<select id="sel_'+lab_order_id+'">';
    wait_lab_result +='<option value="26">ห้องการเงิน (Cashier) [26]</option>';
    if(clinic_type == "T"){
    ...
    wait_lab_result +='</select>';
    wait_lab_result +='<button class="btn btn-info" type="button" onclick="sendQueue(\''+lab_order_id+'\');"> Send Queue</button>';
  //wait_lab_result +='<button class="btn btn-sm btn-warning" type="button" onclick="openSendQueue(\''+lab_order_id+'\');"> ส่งคิว</button>';
...
*/
    ...
    var txt_row = '<tr class="r_data" id="r'+lab_order_id+'" data-id="'+lab_order_id+'" data-uid="'+uid+'"  data-q="'+queue+'" data-room="'+room_no+'" data-prevroom="'+prev_room+'"   >' ;
...
function sendQueueDlg(labOrderID){
  ...
  let sUrl = "lab/mnu_lab_queue_dlg.php?uid="+uid+"&q="+q+"&selroom="+prevroom;
