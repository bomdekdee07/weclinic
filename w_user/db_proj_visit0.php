...
//$visit_note = "";
  ...
  //  uid 	proj_id 	visit_id 	group_id 	schedule_date 	visit_date 	visit_main 	visit_status 	visit_note
           ...
           WHERE uv.uid=? AND uv.proj_id=? AND uv.visit_id=? AND uv.group_id=?  AND uv.visit_date=?
           AND uv.proj_id = v.proj_id AND uv.visit_id=v.visit_id
           AND uv.visit_status = vs.status_id
    ...
    //echo "$proj_id, $uid, $proj_id, $visit_id, $group_id, $visit_date / $query";
           $stmt = $mysqli->prepare($query);
           $stmt->bind_param("ssssssss", $uid, $proj_id, $proj_id, $uid, $proj_id, $visit_id, $group_id, $visit_date);
           if($stmt->execute()){
             $stmt->bind_result($visit_id, $visit_name,$schedule_date,
...
//echo "last visit : $visit_date/$last_visit_date";
    ...
    $schedule_date_change = "เปลี่ยนนัดหมายจาก ". changeToThaiDate($old_schedule_date)." เป็น <b>".changeToThaiDate($new_schedule_date). "</b> โดย $user_id ";
    // set visit_statis = pre counseling
      $query = "UPDATE p_project_uid_visit SET
      schedule_date=?, schedule_date_change=?
      WHERE uid=? AND proj_id=? AND group_id=? AND visit_id=?
      ";
    //  echo "$uid, $proj_id,$group_id, $visit_id /query: $query" ;
      $stmt = $mysqli->prepare($query);
      $stmt->bind_param('ssssss',$new_schedule_date, $schedule_date_change, $uid, $proj_id,$group_id, $visit_id);
      if($stmt->execute()){
           $msg_info += "ได้เปลี่ยนวันนัดหมายแล้ว";
