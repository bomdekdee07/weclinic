...
<div class="card div-uid-visit" id="div_uid_visit_list">
  <div class="card-body">
    <h5 class="card-title"><i class="fa fa-calendar-alt fa-lg" ></i> ตารางนัดหมาย</h5>
...
<div class="card div-uid-visit" id="div_project_visit_info">
  <div class="card-body">
    ...
    <div class="row">
     ...
     <?
        include_once("proj_visit_conf/$proj_id/menu.php");
     ?>
      </div>
  ...
  if(flagSave){
    ...
    $('#cur_proj_final_status_date').val("");
      ...
      if(datalist.length > 0){
        ...
        txt_final_status += '<br><button id="btn_final_status_proj" class="btn btn-info btn-sm" type="button" onclick="openFinalStatus();">';
...
*/
        if (datalist[i+1] !== undefined){
          ...
          //if(dataObj.is_latest_visit == "Y"){

          var visit_remark = dataObj.group_name;
          if(dataObj.schedule_note != "")
          visit_remark += "<br><small><span class='text-danger'>"+dataObj.schedule_note+"</span></small>";
        ...
        //  txt_row += ' <td>'+changeToThaiDate(dataObj.schedule_date)+btn_visit_schedule+'</td>';
          txt_row += ' <td>'+changeToThaiDate(dataObj.schedule_date)+btn_visit_schedule+' <small>'+dataObj.w_period+'</small></td>';
        //  txt_row += ' <td>'+changeToThaiDate(dataObj.visit_date)+btn_visit_meet+'</td>';
          ...
          //txt_row += ' <td>'+dataObj.is_latest_visit+'</td>'; s
...
function openFinalStatus(){
  ...
  if(confirm("ท่านต้องการให้อาสาสมัครสิ้นสุดจากโครงการ ใช่หรือไม่?")){
      ...
      var link = "visit_form/x_"+$("#cur_proj_id").val().toLowerCase()+"_final_status.php?";
      link += "uid="+$("#cur_uid").val(); // uid
      link += "&proj_id="+$("#cur_proj_id").val(); // proj_id
      //link += "&collect_date="+$("#cur_proj_final_status_date").val(); // final status collect date
      link += "&visit_date="+$("#cur_proj_final_status_date").val(); // final status collect date
