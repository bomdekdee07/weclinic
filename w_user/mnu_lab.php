...
<div class="card" id="div_lab_case_list">
  <div class="card-body">
    <div class="card-title">
      ...
      <div class="row">
         <div class="col-sm-6">
           <h5><i class="fa fa-address-book fa-lg" ></i> ตรวจ Lab</h5>
         </div>

         <div class="col-sm-2">
           <label for="ps_trainee_id">ตั้งแต่วันที่:</label>
           <input type="text" id="sel_lab_date_beg" class="form-control" readonly='readonly'>
         </div>
         <div class="col-sm-2">
           <label for="ps_trainee_id">ถึงวันที่:</label>
           <input type="text" id="sel_lab_date_end" class="form-control" readonly='readonly'>
         </div>
         <div class="col-sm-2">
           <label for="btn_search_uid_schedule" class="text-light">.</label>
          <button class="btn btn-info form-control" type="button" id="btn_search_lab"><i class="fa fa-search" ></i> ค้นหา</button>
         </div>
    ...
    <div>
      <table id="tbl_lab_list" class="table table-bordered table-sm table-striped table-hover">
          <thead>
            <tr>
...
$(document).ready(function(){
  ...
  $("#btn_search_lab").click(function(){
    ...
    currentDate.setYear(currentDate.getFullYear() + 543);
      ...
      $("#sel_lab_date_beg").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd/mm/yy',
        onSelect: function(date) {
          $("#sel_lab_date_beg").addClass('filled');
        }
      });
      $("#sel_lab_date_end").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd/mm/yy',
        onSelect: function(date) {
          $("#sel_lab_date_end").addClass('filled');
        }
      });

      $('#sel_lab_date_beg').datepicker("setDate",currentDate );
      $('#sel_lab_date_end').datepicker("setDate",currentDate );

      $('#sel_lab_date_beg').change(function(){
        //alert("change ja");
        //$("#sel_lab_date_end" ).datepicker('setDate', new Date($("#sel_lab_date_beg" ).val()));
      });
...
function selectLabCaseList(){

  var aData = {
            u_mode:"select_in_process_list",
            date_beg:changeToEnDate($('#sel_lab_date_beg').val()),
            date_end:changeToEnDate($('#sel_lab_date_end').val()),
            case_to:"LB"
  };
  save_data_ajax(aData,"w_user/db_case_list.php",selectLabCaseListComplete);

}

function selectLabCaseListComplete(flagSave, rtnDataAjax, aData){
...
//tbl_uid_schedule_list
      ...
      $('#tbl_lab_list > tbody:last-child').append(txt_row);
